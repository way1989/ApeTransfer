...
public class Screen {
    ...
    public static int sp(float sp) {
        if (scaledDensity == 0f)
            scaledDensity = App.getApp().getResources().getDisplayMetrics().scaledDensity;
    ...
    public static int getNavbarHeight() {
        if (hasNavigationBar()) {
            int resourceId = App.getApp().getResources().getIdentifier("navigation_bar_height", "dimen", "android");
            if (resourceId > 0) {
                return App.getApp().getResources().getDimensionPixelSize(resourceId);
            }
        }
        return 0;
    }
